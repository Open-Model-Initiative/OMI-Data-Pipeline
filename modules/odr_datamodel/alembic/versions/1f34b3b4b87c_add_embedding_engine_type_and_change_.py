...
def upgrade() -> None:
    ...
    op.add_column(
        ...
        sa.Column(
            "embedding", Vector(dim=384), nullable=True
        ),
    )
    ...
    op.add_column(
        "content_embeddings",
        sa.Column(
            "embedding", Vector(dim=512), nullable=True
        ),
    )

    op.execute(
        "CREATE TYPE embeddingenginetype AS ENUM ('IMAGE', 'VIDEO', 'VOICE', 'MUSIC', 'TEXT')"
    )
    op.add_column(
        "embedding_engines",
        sa.Column(
            "type",
            sa.Enum(
                "IMAGE", "VIDEO", "VOICE", "MUSIC", "TEXT", name="embeddingenginetype"
            ),
            nullable=True,
        ),
    )
    op.add_column(
        "embedding_engines", sa.Column("supported", sa.Boolean(), nullable=True)
    )
